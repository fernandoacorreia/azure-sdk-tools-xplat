...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .option('-m, --winrm [port]', $('indicates that WinRM should be enabled [5986]'))
    ...
    .execute(function(dnsName, image, userName, password, options, callback) {
      ...
      if (options.winrm) {
        if (typeof options.winrm === 'boolean') {
          options.winrm = 5986;
        } else if ((options.winrm != parseInt(options.winrm, 10)) || (options.winrm > 65535)) {
          callback(new Error($('--winrm [port] must be an integer less than or equal to 65535')));
        }
      }
    ...
    function createDefaultRole(name, callback) {
      ...
      function createDefaultRoleWithPassword_() {
        ...
        if (image.OS.toLowerCase() === 'linux') {
          ...
          if (options.winrm) {
            logger.verbose(util.format($('WinRM is enabled on port %s'), options.winrm));
            inputEndPoints.push({
              Name: 'WinRM',
              Protocol: 'tcp',
              Port: options.winrm,
